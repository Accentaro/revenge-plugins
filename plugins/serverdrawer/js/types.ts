...
export interface NativeStores {
    ...
    GuildStore: { getGuild(id: string): { name: string; icon?: string } | undefined };
    UserStore: { getUser(id: string): NativeUser | undefined };
    GuildReadStateStore: ReadStore; ReadStateStore: ReadStore; UserGuildSettingsStore: object;
    PrivateChannelSortStore: { getPrivateChannelIds(): string[] };
    SelectedChannelStore: { getChannelId(): string | undefined }; SelectedGuildStore: { getGuildId(): string | undefined };
...
export interface NativeExports extends NativeGestureModule {
    ...
    ContextMenuStore: { getState(): { menu?: { items: MenuItem[] } } }; hideContextMenu(): void; showContextMenu: unknown; useContextMenuState: unknown;
    getGuildIconURL(options: { id: string; icon: string; size: number; canAnimate: boolean }): string;
    getChannelIconURL(options: { id: string; icon: string; size: number }): string;
    moveById(source: string | number, target: string | number): void;
    createGuildFolderLocal(ids: string[], name: undefined): void;
    editGuildFolderLocal(id: string | number | undefined, ids: string[], name?: string): void;
